...
function intervalo() {
    ...
    let valor_naoabs = Math.min(...contador) - intervalo
    intIntervalo = Math.abs(valor_naoabs

    )
    console.log("intervalo: ", intIntervalo, intervalo)
    return intIntervalo
}
...
function ultendvalido() {
    let divisao = ip.value.split(".")
    let ultimo_numero = parseInt(divisao[3], 10)
    divisao[3] = (ultimo_numero + 30).toString()
